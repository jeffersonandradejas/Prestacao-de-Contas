...
# Atualizar saldo incluindo receitas extras
...
st.subheader("Subtotais das Receitas Extras")
st.write(f"**Total Receitas Extras:** R$ {receitas_extras_total:.2f}")
...
if st.button("Gerar PDF"):
    ...
    pdf.set_fill_color(*COR_AZUL_CLARO)
    pdf.set_text_color(0)
    pdf.cell(90, 10, "Saldo Anterior", 1, 0, "L", True)
    pdf.cell(90, 10, f"R$ {saldo_anterior:.2f}", 1, 1, "R", True)
    pdf.cell(90, 10, "Saldo Atual", 1, 0, "L", True)
    pdf.cell(90, 10, f"R$ {saldo_atual:.2f}", 1, 1, "R", True)
    pdf.ln(18)
    ...
    st.markdown("[Baixar PDF](prestacao_contas.pdf)")
