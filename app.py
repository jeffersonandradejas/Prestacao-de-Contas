...
receita_extra_texto = st.text_area(
    "Descreva receitas extras (Ex.: Multa R$ 50,00; Aluguel salão R$ 150,00)"
)

valores_receita_extra = re.findall(r"[\d]+[\.,]\d{2}", receita_extra_texto)
receita_extra_total = sum(float(v.replace(",", ".")) for v in valores_receita_extra)

st.write(f"**Total Receitas Extras:** R$ {receita_extra_total:.2f}")
...
despesas_extras_texto = st.text_area(
    "Descreva despesas extras (Ex.: Lâmpada R$ 20,00; Válvula R$ 75,60)"
)

valores_despesas_extras = re.findall(r"[\d]+[\.,]\d{2}", despesas_extras_texto)
despesas_extras_total = sum(float(v.replace(",", ".")) for v in valores_despesas_extras)
